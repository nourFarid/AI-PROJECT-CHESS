...
class DisplayBoard():
    ...
    def __init__(self, board):
        ...
        self.chessBoard = py.transform.scale(
            self.chessBoard, (self.dim, self.dim))
    ...
    def boardlayout_init(self):
        ...
        for x in range(8):
            for y in range(8):
                ...
                self.boardPos[str(chr(97 + x) + str(y + 1))
                              ] = [x * 51.5 + 46, 405 - y * 51]
        ...
        for y in range(8, 0, -1):
            ...
            for char in rawCode:
                if char.isdigit():
                    ...
                    self.boardLayout[str(
                        chr(alphabetCounter)) + str(y)] = self.pieceData(char)
                    alphabetCounter += 1
        self.update_screen()
    ...
    def update_screen(self):
        ...
        for num in self.boardLayout:
            if self.boardLayout[num] != None:
                self.win.blit(self.boardLayout[num].render(
                ), (self.boardPos[num][0], self.boardPos[num][1]))
    ...
    # Except if the second click is on a Piece of same color then that piece becomes the Pieced to be moved
    ...
    def square_select(self, pos):
        ...
        if self.selected_square == None:
            self.square_to_move_too = None
            self.selected_square = str(
                chr(97 + x_board_pos) + str(y_board_pos))
            self.update_possible_moves()
            return None

        else:
            self.square_to_move_too = str(
                chr(97 + x_board_pos) + str(y_board_pos))
...
# From here is only UI stuff to display menus and buttons ect.
    ...
    def game_over_menu(self):
        ...
        while self.game_over:
            ...
            TextSurfMian, TextRectMain = self.text_objects(
                "Game Over", largeText)
            ...
            self.button("Play Again", 225, 300, 100,
                        50, green, bright_green, 4)
...
class Piece():
    ...
    def __init__(self, name, color):
        ...
        if (self.color == "W") or (self.color == "White"):
            if self.name != "Knight":
                self.pieceSurface = py.image.load(
                    "Image\Chess_"+self.name.lower()[0]+"lt60.png")
            else:
                self.pieceSurface = py.image.load("Image\Chess_nlt60.png")
        else:
            if self.name != "Knight":
                self.pieceSurface = py.image.load(
                    "Image\Chess_" + self.name.lower()[0] + "dt60.png")
            else:
                self.pieceSurface = py.image.load("Image\Chess_ndt60.png")

        self.pieceSurface = py.transform.scale(
            self.pieceSurface, (self.dim, self.dim))
